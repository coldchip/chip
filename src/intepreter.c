	...
	while(current != (Op*)list_end(&method->op)) {
		switch(current->op) {
			...
			case OP_LOAD_MEMBER: {
				...
				Object *v2 = load_var(&instance->vars, lookup_constant(current->left));
				if(v2) {
					PUSH_STACK(v2);
				} else {
			...
			case OP_STORE_MEMBER: {
				...
				store_var(&v1->vars, lookup_constant(current->left), v2);
			}
			break;
			case OP_CMPGT: {
				Object *v1 = POP_STACK();
			...
			case OP_SYSCALL: {
				...
				if(strcmp(name->data_string, "print") == 0) {
					...
					write((int)fd->data_number, data->data_string, strlen(data->data_string));
